...
function drawText(canvas, context) {
	...
	context.font = "bold 0.7em HyPost";
	context.textAlign = "left";
	context.fillText("새가 짹짹거립니다.", 10, 20);

	selectObj = document.getElementById("tweets");
	index = selectObj.selectedIndex;
	var tweet = selectObj[index].value;
	context.font = "italic 0.7em 명조";
	
	if (tweet.length > 30) {
		var tweetLines = splitIntoLines(tweet);
		for (var i = 0; i < tweetLines.length; i++) {
			context.fillText(tweetLines[i], 10, 70 + (i * 25));
		}
	} else {
		context.fillText(tweet, 10, 100);
	}

	context.font = "bold 0.7em HyPost";
	context.textAlign = "right";
	context.fillText("훌륭한 문구입니다 !", canvas.width - 10,
			canvas.height - 20);
...
function drawBird(canvas, context) {
	...
	var mirim = new Image();
	twitterBird.src = "Bird.png";mirim.src = "mirim.png";
	twitterBird.onload = function() {
	context.drawImage(twitterBird, 10, canvas.height-40, 30, 30);
